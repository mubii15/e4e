<template>
  <header class="d-flex justify-content-center align-items-center" style="height: 10vh;margin-bottom: 20px;background: #082c32;">
    <div class="d-flex justify-content-center align-items-center" style="height: 100%;object-fit: contain;width: 200px;padding: 10px;overflow: hidden;">
  ...
  methods: {
    ...
    createChart(data) {
      ...
      Highcharts.chart('chart-container', {
        ...
        series: [{
          name: '0-18',
          data: Object.keys(bloodGroups).map(bloodGroup => data.filter(person => person.age <= 18 && person.bloodGroup === bloodGroup).length),
          color: '#7cb5ec'
        }, {
          name: '19-35',
          data: Object.keys(bloodGroups).map(bloodGroup => data.filter(person => person.age > 18 && person.age <= 35 && person.bloodGroup === bloodGroup).length),
          color: '#434348'
        }, {
          name: '36-50',
          data: Object.keys(bloodGroups).map(bloodGroup => data.filter(person => person.age > 35 && person.age <= 50 && person.bloodGroup === bloodGroup).length),
          color: '#90ed7d'
        }, {
          name: '51+',
          data: Object.keys(bloodGroups).map(bloodGroup => data.filter(person => person.age > 50 && person.bloodGroup === bloodGroup).length),
          color: '#f7a35c'
        }]
      });
    }
